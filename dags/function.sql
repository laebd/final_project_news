CREATE OR REPLACE PROCEDURE public.into_table(IN p_path text)
LANGUAGE plpgsql
AS $procedure$
begin
with category_day_count as (
    ...
    from d_news dn 
		group by dn.category, 
             date(to_timestamp(dn.pubdate,'Dy, dd Mon YYYY HH24:MI:SS'))
		), 
    category_day_count_group as (
		select t.category, 
           avg(t.day_count) as day_count_avg, 
            max(t.day_count) as day_count_max
    ...
    category_day_count_group_t as (
		select distinct t.category,
                    t.day_count_avg, 
                    c.d as day_count_max, 
                    t.day_count_max as df
